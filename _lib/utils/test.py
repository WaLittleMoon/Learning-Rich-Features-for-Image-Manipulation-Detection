...
def _get_image_blob(im):
    ...
    for target_size in cfg.FLAGS2["test_scales"]:
        ...
        if np.round(im_scale * im_size_max) > cfg.temps['test_max_size']:
            im_scale = float(cfg.temps['test_max_size']) / float(im_size_max)
...
def apply_nms(all_boxes, thresh):
    ...
    for cls_ind in range(num_classes):
        for im_ind in range(num_images):
            ...
            y2 = dets[:, 3]
            scores = dets[:, 4]
            inds = np.where((x2 > x1) & (y2 > y1) & (scores > cfg.temps['DET_THRESHOLD']))[0]
            dets = dets[inds, :]
            if dets == []:
                continue
...
def test_net(sess, net, imdb, weights_filename, max_per_image=100, thresh=0.05):
    np.random.seed(cfg.temps['rng_seed'])
