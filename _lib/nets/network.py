...
class Network(object):
    ...
    def _proposal_top_layer(self, rpn_cls_prob, rpn_bbox_pred, name):
        with tf.variable_scope(name):
            ...
            rois.set_shape([cfg.temps['rpn_top_n'], 5])
            rpn_scores.set_shape([cfg.temps['rpn_top_n'], 1])
    ...
    def _proposal_target_layer(self, rois, roi_scores, name):
        with tf.variable_scope(name):
            ...
            rois.set_shape([cfg.temps['batch_size'], 5])
            roi_scores.set_shape([cfg.temps['batch_size']])
            labels.set_shape([cfg.temps['batch_size'], 1])
            bbox_targets.set_shape([cfg.temps['batch_size'], self._num_classes * 4])
            bbox_inside_weights.set_shape([cfg.temps['batch_size'], self._num_classes * 4])
            bbox_outside_weights.set_shape([cfg.temps['batch_size'], self._num_classes * 4])
    ...
    def create_architecture(self, sess, mode, num_classes, tag=None, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2)):
        ...
        weights_regularizer = tf.contrib.layers.l2_regularizer(cfg.temps['weight_decay'])#返回一个执行L2正则化的函数
        if cfg.temps['bias_decay']:
